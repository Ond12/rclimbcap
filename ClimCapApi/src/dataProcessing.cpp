...
DataController::DataController(QObject* parent)
    : QObject{ parent }
{
    this->applyRotMatrix = false;
    this->applyOffset = true;
    this->applyWallRotation = false;
...
void DataController::processNewDataPacketPlatformFromNi(const DataPacket& d)
{
    ...
    for (auto gp = m_plaformsList.begin(); gp != m_plaformsList.end(); gp++)
    {
        ...
        //calculate forces
        QGenericMatrix<1, 6, double> result = (*gp).ChannelanalogToForce3axisForcePlat(dataBySensor);
