...
class TableDialog(QDialog):
    def __init__(self, data):
        ...
        table_widget.setHorizontalHeaderLabels(['Sensor Name', 'Sensor ID', 'Compression'])

        for row, (name, sensor_id, compression) in enumerate(data):
            table_widget.setItem(row, 0, QTableWidgetItem(name))
            table_widget.setItem(row, 1, QTableWidgetItem(str(sensor_id)))
            
            table_widget.item(row, 0).setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEnabled)
            table_widget.item(row, 1).setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEnabled)
            ...
            table_widget.setCellWidget(row, 2, combo)
    ...
    def save_data(self):
        for row in range(self.table_widget.rowCount()):
            ...
            file_id = self.table_widget.item(row, 1).text() if self.table_widget.item(row, 1) else ''

            if row == 2:
                compression = self.table_widget.cellWidget(row, 2).currentText()  # Get value from combo box
            else:
                compression = self.table_widget.item(row, 2).text() if self.table_widget.item(row, 2) else ''

            print(f"Row {row}: Name={name}, ID={file_id}, Compression={compression}")
#region window
...
class Wid(QMainWindow):
    def __init__(self):
        ...
        self.parent_folder = os.path.dirname(current_folder)
        
        self.icon_folder = os.path.join(self.parent_folder,'forms/images/svg')
    ...
    def init_actions(self):
        ...
        icon_path = os.path.join(self.icon_folder, 'save-32.svg')
        ...
        icon_path = os.path.join(self.icon_folder, 'plus.svg')
        settings_action = QAction(QIcon(icon_path), "&New run", self)
        ...
        show_cross_hair_action = QAction(QIcon(icon_path), "&Rule", self)
        ...
        self.mtoolbar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
        ...
        if debugmode: toolbar.addAction(debug_data_action)
    ...
    def init_ui(self):
        ...
        self.setWindowTitle('ClimbCap Visu V1')
        ...
        self.videoPlayer_widget.setVisible(False)
    ...
    #Allume le client pour le streaming des données pour le système de sonification
    ...
    #Fusion des forces des capteurs 7 & 8 pour la zone de pied en adhérence 
    ...
    #unused applique une matrice de transformation (rotation) sur tous les capteurs
    ...
    #to do sauvegarde des données brute en interne du logiciel
    def make_back_up_raw_data(self):
        print("make a backup")
    
        #fonction permettant d'enregistrer une configuration du mur
    
    def set_config_action(self):
        json_config_path = os.path.join(self.parent_folder,'data/save.json')
        with open(json_config_path) as json_file:
            data = json.load(json_file)

            self.sensors_id_config_list = []
            for sensor in data['sensors']:
                self.sensors_id_config_list.append(["test",sensor['id'],sensor['isFlip']])
            
            print(self.sensors_id_config_list)
            return self.sensors_id_config_list

    
    #Creation d'une nouvelle acquisition , set up des 11 capteurs plus 2 platformes
    def settings_action(self):
        ...
        self.set_config_action()
        #bugg if not 200 on climbcap-visu warning
        sensor_frequency = 200 
              
        for sensor in self.sensors_id_config_list:
            sensor_id = sensor[1]
            current_sensor = Sensor(sensor_id, 6, sensor_frequency)
            current_sensor.set_is_compression_flip(sensor[2])
    ...
    #fonction de sauvegarde dans le fichier excel
    ...
    #nettoie toutes les données déjà présente
    ...
    #calcule des force normalisee au poids du grimpeur
    ...
    #applique le filtrage sur les forces      
    ...
    #dans le cas ou il n'y a pas de données de chronométrage il est possible de choisir sont propre t0
    ...
    #action principale pour le post traitement des forces
    ...
    #action principale pour la detection des temps de contactes sur les prises
    ...
    #traitement afin de modifier le sens des forces en fonction du sens de montage du capteur
    def flip_action(self):
        ...
        # for sensor in self.sensors_id_config_list:
        #     if sensor[1] == True:
        #         sensorid_compression.append[sensor[0]]
        print("s comp")
        print(sensorid_compression)
    ...
    #detection de la valeur maximum des forces
    ...
    def debug_action(self):
        ...
        self.set_config_action()
        print("debug action")
        sensor_data = self.set_config_action()
        dialog = TableDialog(sensor_data)

        # Show the dialog as modal
        dialog.exec()

    #fonction principale pour le calcul des vitesses
    ...
    #fonction de lecture d'un fichier de force au format excel
    ...
    #utilitaire pour la lecture de la ref capteur
    ...
    #initialisation du serveur udp de reception des données issue de climbcap-data 
    ...
    #mise a jour de l'affichage des graphs
    ...
    #initialisation du serveur de sonification
